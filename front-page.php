<?php get_header();?>


<div class="content-box"><?php the_content(); ?></p>

  <div class="banner">
    <div class="row">
  <div class="two columns"><img class="banner-image hidden-image" src="<?php echo get_stylesheet_directory_uri();?>/images/nia-callig.jpg" alt=""></div>
  <div class="two columns"><img class="banner-image hidden-image" src="<?php echo get_stylesheet_directory_uri();?>/images/5pointz_BIG.jpg" alt=""></div>
  <div class="two columns"><img class="banner-image" src="<?php echo get_stylesheet_directory_uri();?>/images/20170803_192237.jpg" alt=""></div>
  <div class="two columns"><img class="banner-image" src="<?php echo get_stylesheet_directory_uri();?>/images/IMG-0366.jpg" alt=""></div>
  <div class="two columns"><img class="banner-image" src="<?php echo get_stylesheet_directory_uri();?>/images/20180924_103108.jpg" alt=""></div>
  <div class="two columns"><img class="banner-image hidden-image" src="<?php echo get_stylesheet_directory_uri();?>/images/20201004_112719.jpg" alt=""></div>


</div>
  </div>

<div class="button-div">
  <a class="scroll-down-buttons button" href="#ux-design">UX Design/Frontend Web Development</a>
  <a class="scroll-down-buttons button" href="#learning-design">Learning Design/Language Tutoring</a>
  <a class="scroll-down-buttons button" href="#project-management">Project Management</a>
  <a class="scroll-down-buttons button" href="#other-projects">Other Projects</a>
</div>

</div>


<h5 class="scroll-text">Scroll (or click a button) to see more!</h3>

<div class="section-box" id="ux-design">
  <h1 class="section-box-title">UX Design/Frontend Web Development</h1>
  <div class="section-box-links row">
    <div class="section-box-link left six columns">
      <img class="section-box-image" src="<?php echo get_stylesheet_directory_uri();?>/images/Screen Shot 2021-08-26 at 6.03.26 PM.png" alt="">
     <a href="#">  <div class="section-box-overlay">
        <div class="section-box-text">
          Stages Learning Line Blog
        </div>
      </div></a>
    </div>
    <div class="section-box-link right six columns">
      <img class="section-box-image" src="<?php echo get_stylesheet_directory_uri();?>/images/Screen Shot 2022-06-23 at 6.24.10 PM.png" alt="">
      <a href="#">  <div class="section-box-overlay">
        <div class="section-box-text">
          This website!
        </div>
      </div></a>
    </div>
  </div>
  <a class="scroll-up-buttons button button-primary" href="#">See More</a>
  <a class="scroll-up-buttons button" href="#">Back to top</a>
  <a class="scroll-up-buttons button" href="#learning-design">Next</a>
      </div>


<div class="section-box" id="learning-design">
  <h1 class="section-box-title">Learning Design/Language Tutoring</h1>
  <div class="section-box-links row">
    <div class="section-box-link left six columns">
      <img class="section-box-image" src="<?php echo get_stylesheet_directory_uri();?>/images/Screen Shot 2022-06-23 at 6.30.35 PM.png" alt="">
     <a href="#">  <div class="section-box-overlay">
        <div class="section-box-text">
          Private Mandarin and Japanese Tutoring
        </div>
    </div></a>
  </div>
    <div class="section-box-link right six columns">
      <img class="section-box-image" src="<?php echo get_stylesheet_directory_uri();?>/images/Screen Shot 2022-06-23 at 6.28.08 PM.png" alt="">
    <a href="#">  <div class="section-box-overlay">
        <div class="section-box-text">italki.com Tutoring</div>
      </div> </a>
    </div>

</div>
  <a class="scroll-up-buttons button button-primary" href="#">See More</a>
  <a class="scroll-up-buttons button" href="#">Back to top</a>
    <a class="scroll-up-buttons button" href="#project-management">Next</a>
      </div>




    <div class="section-box" id="project-management">
      <h1 class="section-box-title">Project Management</h1>

      <div class="section-box-links row">
        <div class="section-box-link left six columns">
          <img class="section-box-image" src="<?php echo get_stylesheet_directory_uri();?>/images/Screen Shot 2022-06-24 at 2.27.52 PM.png" alt="">
          <a href="#"> <div class="section-box-overlay">
            <div class="section-box-text">
              Throughline (Harvard Graduate School of Education student organization)
            </div>
          </div> </a>
        </div>

        <div class="section-box-link right six columns">
          <img class="section-box-image" src="<?php echo get_stylesheet_directory_uri();?>/images/Screen Shot 2022-06-24 at 2.20.39 PM.png" alt="">
        <a href="#">  <div class="section-box-overlay">
            <div class="section-box-text">
              Civic Spirit
            </div>
        </div> </a>
      </div>
          </div>
        <a class="scroll-up-buttons button button-primary" href="#">See More</a>
        <a class="scroll-up-buttons button" href="#">Back to top</a>
          <a class="scroll-up-buttons button" href="#other-projects">Next</a>
        </div>

        <div class="section-box" id="other-projects">
          <h1 class="section-box-title">Other Projects</h1>

          <div class="section-box-links row">
            <div class="section-box-link left six columns">
              <img class="section-box-image" src="<?php echo get_stylesheet_directory_uri();?>/images/Screen Shot 2022-06-24 at 2.24.52 PM.png" alt="">
            <a href="#">   <div class="section-box-overlay">
                <div class="section-box-text">
                  "What Does America Mean To You?" - Short Documentary
                </div>

            </div> </a>
          </div>



            <div class="section-box-link right six columns">
              <img class="section-box-image" src="<?php echo get_stylesheet_directory_uri();?>/images/Screen Shot 2022-06-24 at 2.26.52 PM.png" alt="">
            <a href="#">  <div class="section-box-overlay">
                <div class="section-box-text">
                  "Constructive Conversation: An Educator's Handbook for Addressing Race"
                </div>
            </div> </a>

          </div>
        </div>

  <a class="scroll-up-buttons button button-primary" href="#">See More</a>
  <a class="scroll-up-buttons button" href="#">Back to top</a>
        </div>


<?php get_footer();?>
